...
import { isConnected, getPRLabel, getPRLabelForProvider } from "../store/providers/reducer";
...
export const CreatePullRequestPanel = props => {
	...
	const [prNameWithOwner, setPrNameWithOwner] = useState("");
	...
	// const [headForkedRepo, setHeadForkedRepo] = useState<{
	// 	id: string;
	...
	const prLabel = useMemo(() => {
		return prProviderId ? getPRLabelForProvider(prProviderId) : getPRLabel(derivedState.state);
	}, [prProviderId, derivedState.state]);
	...
	const fetchPreconditionData = async (isRepoUpdate = false) => {
		...
		try {
			...
			if (result && result.success) {
				...
				if (result.isFork) {
					setAcrossForks(true);
					await fetchRepositoryForks(result.providerId!, result.remoteUrl!);
				}
	...
	useEffect(() => {
		fetchRepositoryForks(prProviderId, prRemoteUrl);
	...
	const renderCompareBranchesAcrossForksDropdown = () => {
		...
		if (items.length >= 10) {
			items.unshift({ label: "-" } as any);
			(items as any[]).unshift({ type: "search", placeholder: "Search...", action: "search" });
		}
		return (
			<DropdownButton variant="secondary" items={items}>
